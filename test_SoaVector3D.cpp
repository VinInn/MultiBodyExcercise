...
template<typename V, typename Cont>
float weight(V const & v, Cont const & s) {
  auto n = s.size();
  float a = 0.;
  for (auto i=0U; i<n; ++i) {
    a+= dist2(v,s[i]);
...
int goExt() {
  ...
  std::cout << "extvect" << std::endl;

  // using namespace extvec;

  using V3D = extvec::Vec4D<float>;
  using R3D = extvec::Vec4D<float>;
  ...
  AVector<V3D> s(1280); auto n = s.size(); s[0]=k1;
  for (auto i=1U; i<n; ++i) s[i] = s[i-1]+k1;
